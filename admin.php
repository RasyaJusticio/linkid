  ...
  switch ($_GET['view']) {
    ...
    case 'cities':
      ...
      switch ($_GET['sub_view']) {
        case '':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Cities"));
        ...
        case 'edit':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT * FROM system_cities WHERE city_id = %s", secure($_GET['id'], 'int')));
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();

          $province_get_data = $db->query("SELECT province_id, province_name FROM system_provinces");
          if ($province_get_data->num_rows > 0) {
            while ($row = $province_get_data->fetch_assoc()) {
              $provinces[] = $row;
            }
          }
          ...
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Cities") . " &rsaquo; " . $data['city_name']);
          break;

        case 'add':
          // get data
          $provinces_get_data = $db->query("SELECT province_id, province_name FROM system_provinces");
          if ($provinces_get_data->num_rows > 0) {
            while ($row = $provinces_get_data->fetch_assoc()) {
              $provinces[] = $row;
            }
          }
          ...
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Cities") . " &rsaquo; " . __("Add New"));
          break;
